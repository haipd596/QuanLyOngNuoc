...
export const createRadioGroupField = (
...
) => new Field<ViewRadioGroupProps>({
  fieldName: FIELD_NAME.RADIO_GROUP,
  key: '',
  version: 0,
  componentPropsAllowConfig: {
    ...
    isShowOtherInfo: new ConfigBasic({
      type: CONFIG_BASIC_FIELD_TYPE.BOOLEAN,
      props: {
        defaultValue: false,
      },
    }),
    labelError: new ConfigBasic({
      type: CONFIG_BASIC_FIELD_TYPE.STRING,
      props: {
        defaultValue: '',
      },
    }),
    // moreInfo: new ConfigBasic({
    //   type: CONFIG_BASIC_FIELD_TYPE.STRING,
    //   props: {
    //     defaultValue: '',
    //   },
    // }),
    optionsMoreInfo: new ConfigBasic({
      type: CONFIG_BASIC_FIELD_TYPE.ARRAY,
      props: {
        defaultValue: [
          {
