...
import { TAsyncViewInvestorShortenProps } from '../type';
import Individual from './Individual';
import Org from './Org';

const ViewerInvestorShorten: React.FC<TAsyncViewInvestorShortenProps> = (props) => {
  const {
    fieldKey,
    onChange,
    value,
              ...
              <div key={field.key}>
                ...
                >
                  {(values:any) => {
                    ...
                    return (
                      <>
                        {showCaNhan && type === "CaNhan" && <Individual index={field.name} />}
                        {showToChuc && type === 'ToChuc' && <Org index={field.name}/>}
                        {!showToChuc && showCaNhan && !type && <Individual index={field.name} />}
                        {!showCaNhan && showToChuc && !type && <Org index={field.name}/>}
                      </>
                    )
                  }}
                </Form.Item>
...
export default ViewerInvestorShorten;
