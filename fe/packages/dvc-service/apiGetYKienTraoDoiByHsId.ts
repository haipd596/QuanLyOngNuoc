...
export const apiGetYKienTraoDoiByHsId = async (TitleOpinion?: string) => {
  const params = new URL(window.location.href).searchParams;
  const hsid = params.get(HOSO_ID_PARAM1) || params.get(HOSO_ID_PARAM2);
  const url = `${getBaseDvcApi()}/_api/web/lists/getbytitle('mc_YKienTraoDoi')/items?$select=*&$filter=HoSoIdId eq '${hsid}' and Title eq '${TitleOpinion}'`;
